...
// End page if not frontend
if( is_admin() ) :
	return ;
endif;


// Force enable mega walker
if( !class_exists( 'FW_Ext_Mega_Menu_Walker' ) && jevelin_framework() == 'redux' ) :
	class FW_Ext_Mega_Menu_Walker { }
endif;


// Imitate Unyson framework funtion
if( !function_exists( 'fw_ext_mega_menu_get_meta' ) ) :
	function fw_ext_mega_menu_get_meta( $id, $name = '' ) {
		if( $name == 'enabled' ) :
			$mega_menu = get_post_meta( $id, 'mega-menu' );
...
if( !function_exists( 'fw_ext_mega_menu_get_db_item_option' ) ) :
	function fw_ext_mega_menu_get_db_item_option( $id, $name = '' ) {
		$name = 'jevelin-' . str_replace( 'row/', '', $name );
		$name = str_replace( '_', '-', $name );
		$value = get_post_meta( $id, $name, 1 );
		if( $value ) :
			return $value;
		endif;
	}
...
// Replace Unyson mega menu render function
function jevelin_mega_menu_render_item( $params ) {
	extract( $params );

	$attributes_out = [];
	if( is_array( $attributes ) && count( $attributes ) ) :
		foreach( $attributes as $attribute_key => $attribute ) :
			$attributes_out[] = esc_attr( $attribute_key ) . ' = "'. esc_attr( $attribute ) .'"';
		endforeach;
	endif;

	$o = $args->before;
	$o.= '<a '.implode( ' ', $attributes_out ).'>' . ( $args->link_before . $title . $args->link_after ) . '</a>';
	$o.= $args->after;
	return $o;
}
...
if( class_exists( 'FW_Ext_Mega_Menu_Walker' ) ) {
	...
	# END - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if( fw_ext_mega_menu_get_meta( $item->ID, 'enabled' ) ) :
				$this->mega_menu_ids[$item->ID] = true;
				$class_names.= ' menu-item-has-mega-menu';
			...
			// Mega menu column
			if( isset( $this->mega_menu_ids[ $item->menu_item_parent ] ) ) :
				$class_names.= ' mega-menu-col';
			endif;

			// Icon
			$icon_class = '';
			if( $depth == 0 && fw_ext_mega_menu_get_meta( $item->ID, 'icon' ) ) :
				$icon_class = fw_ext_mega_menu_get_meta( $item->ID, 'icon' );
			endif;
	...
	# BEGIN - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
			...
			if( function_exists( 'fw_ext' ) && jevelin_framework() == 'unyson' ) :
				$item_output = fw_ext('megamenu')->render_str('item-link', compact('item', 'attributes', 'title', 'args', 'depth'));
			else :
				$item_output = jevelin_mega_menu_render_item( compact('item', 'attributes', 'title', 'args', 'depth') );
			endif;
	# END - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
