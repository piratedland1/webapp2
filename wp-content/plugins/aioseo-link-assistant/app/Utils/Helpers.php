...
use AIOSEO\Plugin\Addon\LinkAssistant\Traits as Traits;
...
class Helpers {
	...
	/**
	 * Returns the links/suggestion and their totals for the given post.
	 *
	 * @since 1.0.0
	 *
	...
	private function getPostLinksHelper( $postId, $type, $limit = 20, $offset = 0 ) {
		...
		switch ( $type ) {
			case 'inboundInternal':
				$total = Models\Link::getTotalInboundInternalLinks( $postId );
			...
			case 'outboundInternal':
				$total = Models\Link::getTotalOutboundInternalLinks( $postId );
			...
			case 'affiliate':
				$total = Models\Link::getTotalAffiliateLinks( $postId );
			...
			case 'external':
				$total = Models\Link::getTotalExternalLinks( $postId );
	...
	public function getSuggestionsScanPercent() {
		...
		$remainingScanCount = aioseo()->core->db->start( 'posts as p' )
		...
		$allPostsCount = aioseo()->core->db->start( 'posts as p' )
			->whereRaw( "p.post_status IN ( $implodedPostStatuses )" )
			->whereRaw( "p.post_type IN ( $implodedPostTypes )" )
			->where( 'p.post_name !=', '' )
			->count();

		if ( 0 === $allPostsCount || 0 === $remainingScanCount ) {
			return 100;
		}

		return round( 100 - 100 * ( $remainingScanCount / $allPostsCount ) );
	}
	...
	public function isScannablePost( $post ) {
		...
		if ( ! aioseo()->helpers->isValidPost( $post, aioseo()->helpers->getPublicPostStatuses( true ) ) ) {
			return false;
		}
