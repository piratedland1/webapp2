...
function crawlomatic_file_get_html(
	$url,
	...
	$defaultSpanText = DEFAULT_SPAN_TEXT)
{
	...
	$dom = new crawlomatic_simple_html_dom(
		null,
		$lowercase,
		$forceTagsClosed,
		$target_charset,
...
function crawlomatic_str_get_html(
	$str,
	$lowercase = true,
	$forceTagsClosed = true,
	$target_charset = DEFAULT_TARGET_CHARSET,
...
function crawlomatic_dump_html_tree($node, $show_attr = true, $deep = 0)
{
	$node->dump($node);
}

class crawlomatic_crawlomatic_simple_html_dom_node
{
...
class crawlomatic_simple_html_dom
{
	public $root = null;
	public $nodes = array();
		...
		$defaultSpanText = DEFAULT_SPAN_TEXT)
	{
		...
		$this->root = new crawlomatic_crawlomatic_simple_html_dom_node($this);
		$this->root->tag = 'root';
	...
	protected function parse()
	{
		while (true) {
			...
			$node = new crawlomatic_crawlomatic_simple_html_dom_node($this);
			++$this->cursor;
	...
	{
		return @crawlomatic_str_get_html("<$name>$value</$name>")->firstChild();
	}

	function createTextNode($value)
	{
		return @end(crawlomatic_str_get_html($value)->nodes);
	}
