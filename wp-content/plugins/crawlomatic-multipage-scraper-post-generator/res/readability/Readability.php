...
class Readability
{
    ...
    public function parse($html)
    {
        ...
        while (true) {
            ...
            $length = readability_mb_strlen(preg_replace(NodeUtility::$regexps['onlyWhitespace'], '', $result->textContent));
    ...
    private function loadHTML($html)
    {
        ...
        if ($this->configuration->getParser() === 'html5') {
            ...
            if ($this->configuration->getNormalizeEntities() && function_exists('mb_convert_encoding')) {
        ...
        foreach ($this->dom->getElementsByTagName('meta') as $meta) {
            ...
            if ($elementProperty) {
                if (preg_match($propertyPattern, $elementProperty, $matches)) {
                    $name = preg_replace('/\s/', '', readability_mb_strtolower($matches[0]));
                    // multiple authors
            ...
            if (!$matches && $elementName && preg_match($namePattern, $elementName)) {
                ...
                if ($content) {
                    ...
                    $name = preg_replace(['/\s/', '/\./'], ['', ':'], readability_mb_strtolower($name));
                    $values[$name] = trim($content);
                }
            }
        }
    ...
    private function getArticleTitle()
    {
        ...
        } elseif (readability_mb_strlen($curTitle) > 150 || readability_mb_strlen($curTitle) < 15) {
    ...
    private function textSimilarity(string $textA, string $textB) {
        $tokensA = array_filter(preg_split(NodeUtility::$regexps['tokenize'], readability_mb_strtolower($textA)));
        $tokensB = array_filter(preg_split(NodeUtility::$regexps['tokenize'], readability_mb_strtolower($textB)));
        ...
        $slen = readability_mb_strlen(implode(' ', $tokensB));
        if($slen == 0)
        {
            return 0;
        }
        $distanceB = readability_mb_strlen(implode(' ', $uniqTokensB)) / $slen;
        return 1 - $distanceB;
    }
    ...
    private function isValidByline($text)
    {
        if (gettype($text) == 'string') {
            ...
            return (readability_mb_strlen($byline) > 0) && (readability_mb_strlen($byline) < 100);
        }
    ...
    private function rateNodes($nodes)
    {
        ...
        foreach ($nodes as $node) {
            ...
            if (readability_mb_strlen($node->getTextContent(true)) < 25) {
                continue;
            }
            ...
            $contentScore += min(floor(readability_mb_strlen($node->getTextContent(true)) / 100), 3);
        ...
        for ($i = 0; $i < $siblings->length; $i++) {
            ...
            if ($sibling === $topCandidate) {
                ...
                if ($sibling->contentScore + $contentBonus >= $siblingScoreThreshold) {
                    ...
                    if (readability_mb_strlen($nodeContent) > 80 && $linkDensity < 0.25) {
                        $append = true;
                    } elseif ($nodeContent && readability_mb_strlen($nodeContent) < 80 && $linkDensity === 0 && preg_match('/\.( |$)/', $nodeContent)) {
                        $append = true;
                    }
                }
            }
    ...
    public function prepArticle(DOMDocument $article)
    {
        ...
        foreach ($article->childNodes as $child) {
            $this->_cleanMatchedNodes($child, function ($node, $matchString) use ($shareElementThreshold) {
                return (preg_match(NodeUtility::$regexps['shareElements'], $matchString) && readability_mb_strlen($node->textContent) < $shareElementThreshold);
            });
        }
        ...
        if ($h2->length === 1) {
            $lengthSimilarRate = (readability_mb_strlen($h2->item(0)->textContent) - readability_mb_strlen($this->getTitle())) / max(readability_mb_strlen($this->getTitle()), 1);
    ...
    public function _fixLazyImages(DOMDocument $article)
    {
        ...
        foreach ($images as $elem) {
            ...
            // attribute value.
            if (($elem->getAttribute('src') || $elem->getAttribute('srcset')) && readability_mb_stripos($elem->getAttribute('class'), 'lazy') === false) {
                continue;
            }
    ...
    private function getTextDensity($e, array $tags) {
        $textLength = readability_mb_strlen($e->getTextContent(true));
        if ($textLength == 0) {
            return 0;
        }
        $childrenLength = 0;
        ...
        foreach ($children as $child) {
            $childrenLength += readability_mb_strlen($child->getTextContent(true));
        }
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            if (!$isList) {
                ...
                array_walk($listNodes, function ($list) use(&$listLength) {
                    $listLength += readability_mb_strlen($list->getTextContent());
                });
                $nodeTextLength = readability_mb_strlen($node->getTextContent());
                if (!$nodeTextLength) {
                    $isList = true;
                } else {
            ...
            */
            ...
            if (substr_count($node->getTextContent(false), ',') < 10) {
                ...
                $contentLength = readability_mb_strlen($node->getTextContent(true));
