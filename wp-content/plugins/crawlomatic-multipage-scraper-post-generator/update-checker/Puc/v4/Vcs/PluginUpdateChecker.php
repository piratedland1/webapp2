	...
	class Puc_v4_Vcs_PluginUpdateChecker extends Puc_v4_Plugin_UpdateChecker implements Puc_v4_Vcs_BaseChecker {
		...
		public function requestInfo($unusedParameter = null) {
			...
			if ( empty($info->sections['changelog']) ) {
				...
				if ( empty($info->sections['changelog']) ) {
					$info->sections['changelog'] = __('There is no changelog available.');
				}
			}
		...
		protected function readmeTxtExistsLocally() {
			...
			if ( empty($this->pluginAbsolutePath) || !$wp_filesystem->is_dir($pluginDirectory) || ($pluginDirectory === '.') ) {
				return false;
			}
			return $wp_filesystem->is_file($pluginDirectory . '/readme.txt');
		}
