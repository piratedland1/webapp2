...
if ( !class_exists('Puc_v4_Plugin_UpdateChecker', false) ):
	...
	class Puc_v4_Plugin_UpdateChecker extends Puc_v4_UpdateChecker {
		...
		protected function getPluginHeader() {
			...
			if ( !$wp_filesystem->is_file($this->pluginAbsolutePath) ) {
		...
		public function displayManualCheckResult() {
			if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
				...
				if ( $status == 'no_update' ) {
					$message = __('This plugin is up to date.');
				} else if ( $status == 'update_available' ) {
					$message = __('A new version of this plugin is available.');
				} else {
					$message = sprintf(__('Unknown update checker status "%s"'), htmlentities($status));
				}
				printf(
