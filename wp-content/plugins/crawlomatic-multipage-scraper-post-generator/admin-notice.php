...
if ( ! class_exists( 'Crawlomatic_Admin_Notices' ) ) {
    class Crawlomatic_Admin_Notices {
        ...
        public function action_admin_init() {
            $dismiss_option = filter_input( INPUT_GET, 'crawlomatic_dismiss', FILTER_UNSAFE_RAW );
            if ( is_string( $dismiss_option ) ) {
                update_option( "crawlomatic_dismissed_$dismiss_option", true );
                wp_die();
            }
        }
        ...
        public function action_admin_enqueue_scripts() {
            wp_enqueue_script(
                'crawlomatic-notify',
                plugins_url( 'scripts/notice.js', __FILE__ ),
                array( 'jquery' )
            );
        }
            ...
            foreach ( explode( ',', self::TYPES ) as $type ) {
                foreach ( $this->admin_notices->{$type} as $admin_notice ) {
                    ...
                    $dismiss_url = add_query_arg( array(
                        'crawlomatic_dismiss' => $admin_notice->dismiss_option
                    ), admin_url() );

                    if ( ! get_option( "crawlomatic_dismissed_{$admin_notice->dismiss_option}" ) ) {
                        ?><div
                            class="notice crawlomatic-notice notice-<?php echo esc_html($type);
