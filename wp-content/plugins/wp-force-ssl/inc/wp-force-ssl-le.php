...
class WP_Force_SSL_LetsEncrypt
{
    function generate_ssl_certificate($email, $domain, $challenge_dir, $ssl_dir)
    {
        ...
        if ($order->allAuthorizationsValid()) {
            ...
            $wp_force_ssl->generate_certificate_log('LetsEncrypt: Authorizations could not be verified. Please check that ' . trailingslashit($domain) . '.well-known/acme-challenge/' . $challenge['filename'] . ' is publicly accessible', true);
            return new WP_Error(1, 'LetsEncrypt: Domain verification failed. Please check that <strong>' . trailingslashit($domain) . '.well-known/acme-challenge/' . $challenge['filename'] . '</strong> is publicly accessible');
        }
    }
